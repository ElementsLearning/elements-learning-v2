      ...
      {flowchartSteps.map(({name, color, corner, optionals}, i) =>
        <>
          <SlidingDiv  direction={"top"} delay={i*0.3} className="flex md:flex-col justify-center ">
            {optionals && 
            <div className="flex-1  md:h-full flex md:flex-col items-center h-64 w-full">
              ...
              <div className="flex-1 flex md:flex-col justify-around">
                {Array.from({length: 10}).map((_, i) => <div key={`Dot-${i}`} className="size-0.5 sm:size-1 rounded-full bg-[#F3EEE8]" />)}  
              </div>
            </div>}
            <div className={`flex p-2 size-36 sm:p-4 sm:w-40 xl:p-8 xl:w-64 md:h-1/4 lg:h-1/3 lg:p-6 lg:w-48 justify-center items-center text-center rounded-[30px] drop-shadow-md`}
            style={{backgroundColor: color}}>
              <p className="font-bold text-sm sm:text-base md:text-xl xl:text-2xl">{name}</p>
            </div>
            {optionals && 
            <div className="flex-1 flex md:flex-col items-center h-64 w-full">
              <div className="flex-1 flex md:flex-col justify-around">
                {Array.from({length: 10}).map((_, i) => <div key={`VDot-${i}`} className="size-0.5 sm:size-1 rounded-full bg-[#F3EEE8]" />)} 
              </div>
          ...
          <SlidingDiv direction={"left"} delay={i*0.3 + 0.1} className="flex-1 flex flex-col gap-1 md:flex-row items-center justify-around h-64 w-full">
            {Array.from({length: 6}).map((_, i) => <div key={`dots-${i}`} className="size-1 xl:size-1.5 bg-[#F3EEE8] rounded-full" />)}   
          </SlidingDiv>
          }
        </>
      )}
