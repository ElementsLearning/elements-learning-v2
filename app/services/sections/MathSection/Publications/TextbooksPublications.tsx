...
export const TextbooksPublications = () => {
  const [opened, setOpened] = useState<"OVERVIEW" |  "STUDENTS">("OVERVIEW")
  ...
  const texts = {
    ...
    // </>,
    "STUDENTS": <>
      <h3 className="text-left ttext-xs sm:text-sm md:text-base lg:text-lg xl:text-xl text-[#FBBA41] font-bold">Our Student Workbooks/ Game Boards are designed to complement the Textbooks, providing students with practical exercises and activities that reinforce their learning.</h3>
      <div className="pl-12 md:pl-24 lg:pl-32 xl:pl-40">
        <BulletList contentClass="text-sm text-left md:text-base xl:text-lg"
        list={[
  ...
  return (
    ...
    //         <button onClick={() => setOpened(opened === "OVERVIEW" ? "STUDENTS" : "OVERVIEW")} className="lg:px-6 lg:py-4 p-2 font-bold md:text-lg lg:text-lg text-white bg-[#FBBA42]">{opened === "STUDENTS" ? "Overview" : "Student Workbooks"}</button>
    //       </SlidingDiv>
    //     </SlidingDiv>
  ...
  <div className="flex flex-col-reverse sm:flex-row gap-2">
    <SlidingDiv direction={"top"} className="flex-[3_0_0] flex flex-col text-right p-4 md:p-8 lg:p-10 xl:p-12 gap-4 lg:gap-6 xl:gap-8 bg-[#F3EEE8]">
      ...
      <SlidingDiv direction={"bottom"} className="flex gap-2 mt-auto justify-end">
        <button 
          onClick={() => setOpened("OVERVIEW")} 
          className={`lg:px-6 lg:py-4 p-2 font-bold md:text-lg lg:text-lg text-white ${
            opened === "OVERVIEW" ? "bg-black" : "bg-[#FBBA42]"
          }`}
        >
          Textbooks
        </button>
        <button 
          onClick={() => setOpened("STUDENTS")} 
          className={`lg:px-6 lg:py-4 p-2 font-bold md:text-lg lg:text-lg text-white ${
            opened === "STUDENTS" ? "bg-black" : "bg-[#FBBA42]"
          }`}
        >
          Game Boards
        </button>
